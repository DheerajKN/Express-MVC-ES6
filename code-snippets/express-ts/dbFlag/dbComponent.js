...
module.exports.addDBComponent = (arguement, folderDirectory, tableName) => {
  if(/^(mongo|true)$/.test(arguement.db)){
    ...
    let packageFile = `${folderDirectory}/app.ts`
      ...
      fetchContent('/'+join('code-snippets', 'express-ts', 'dbFlag', 'sequelize.connect.ts')).then((data)=>
        createFileWithContent(folderDirectory + '/src/models/index.ts', data))

      fetchContent('/'+join('code-snippets', 'express-ts', 'dbFlag', 'userschema.sequelize.ts')).then((data)=>
        createFileWithContent(folderDirectory + '/src/models/User.ts', data))
